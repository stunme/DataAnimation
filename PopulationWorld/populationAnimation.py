...
def create_animation(df):
    ...
    def animate(frame):
        ...
        # main bar 
        top_country = df.loc[((df['Time'] == frame))].nlargest(10,columns='Population').sort_values('Population',ascending=True) 
        # if top_country.size == 0:
            
        ax.barh(top_country['Location'], 
                top_country['Population'],
                alpha = 0.3)

        # element for each country
        for i, row in top_country.iterrows():
